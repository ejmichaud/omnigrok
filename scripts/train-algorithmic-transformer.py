...
from omnigrok.data import ArithmeticDataset
from omnigrok.transformer import Transformer

import seml
from sacred import Experiment
from sacred.utils import apply_backspaces_and_linefeeds
ex = Experiment("train-algorithmic-transformer")
...
def run(operators,
        ...
        embedding_initialization_scale,
        ...
        for equation, answer in islice(cycle(train_loader), optimization_steps):
            ...
            if steps % log_freq == 0:
                with torch.no_grad():
                    ...
                    ex.info['l2'].append(float(np.sqrt(embedding_l2.item() + decoder_l2.item() + last_layer_l2.item())))
                    ex.info['decoder_l2'].append(float(np.sqrt(decoder_l2.item() + last_layer_l2.item())))
                    ex.info['last_layer_l2'].append(float(np.sqrt(last_layer_l2.item())))
